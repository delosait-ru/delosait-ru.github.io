...
$(window).scroll(function () {
  if ($(this).scrollTop() > 220) {
    $('.heading').addClass('mfixed');
  } else {
    $('.heading').removeClass('mfixed');
  }
});
$(window).scroll(function () {
  if ($(this).scrollTop() > 220) {
    $('header-page').addClass('pfixed');
  } else {
    $('header-page').removeClass('pfixed');
  }
});
/*Мега меню*/
...
$(".content-block").each(function () {
  ...
  var hide = $(this).find(".hide-content");
  hide.hide();
  more.click(function () {
    hide.slideToggle();
    more.text(more.text() == "Скрыть" ? "Показать еще 15" : "Скрыть");
  });
});
