...
$('.ui-quantity input').bind('input propertychange', function () {
  var $this = $(this);
  if ($this.val().length == 0 || parseInt($this.val()) <= 0) $this.val(1 + " " + $(this).data('units'));
});
...
$(window).scroll(function () {
  if ($(this).scrollTop() > 220) {
    $('.header__middle').addClass('mfixed');
  } else {
    $('.header__middle').removeClass('mfixed');
  }
});
$(function () {
  $('.planes-slider').slick({
...
/*Аккордион с преимуществами в мобильной версии*/
